...
final class ZoneEngine: ObservableObject {
    ...
    /// Always starts the enforcer — if accessibility isn't granted, AX calls
    /// will silently fail but dividers still show. No gating on permission checks
    /// since AXIsProcessTrustedWithOptions can return stale/incorrect results
    /// for re-exported or non-/Applications apps.
    func activate(with profile: Profile, stickyEdgesEnabled: Bool, stickyEdgeThreshold: Double) {
        ...
        // Always start — let AX calls fail silently if permissions aren't there
